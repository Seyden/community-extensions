...
var _Sources = (() => {
  ...
  // node_modules/simple-url/url.js
  var require_url = __commonJS({
    "node_modules/simple-url/url.js"(exports, module) {
      "use strict";
      ...
      var qs = require_lib3();
      var urlRegex = /^(?:([^:/?#]+):(?=\/\/))?(?:\/\/(?:(.*?)@)?([^/?#]*)?)?([^?#]*)(?:\?([^#]*))?(?:#(.*))?/;
      var parseUrl = function parseUrl2(url2, parseQuery) {
        var match = urlRegex.exec(url2);
        return match && {
          protocol: match[1] || "",
          auth: match[2] || "",
          host: match[3] || "",
      ...
      var url = {
        ...
        trimOrigin: function trimOrigin(url2) {
          url2 = url2 || "";
          url2 = url2.replace(/(.*?)\/\/(.*?)\//, "");
          url2 = url2.charAt(0) === "/" ? url2 : "/" + url2;
          return url2;
        },
        qs
      };
  ...
  function extractMangaData(text, node) {
  ...
  var AsuraScansParser = class {
    ...
    parseMangaDetails(data, mangaId, source) {
      ...
      if (obj == "") {
        throw new Error(`Failed to parse comic object for manga ${mangaId}`);
      }
      ...
      const tagSections = [
        App.createTagSection({
          id: "0",
          label: "genres",
          tags: comicObj.comic.genres.map((tag) => App.createTag({ id: `genres:${tag.id.toString()}`, label: tag.name }))
        })
      ];
    ...
    async parseChapterList(data, mangaId, source) {
      ...
      if (obj == "") {
        throw new Error(`Failed to parse chapters object for manga ${mangaId}`);
      }
    ...
    parseTags(data) {
      ...
      filters.types.forEach((type) => {
        tagSections[3].tags.push(App.createTag({ id: `type:${type.id.toString()}`, label: type.name }));
      });
      filters.genres.forEach((type) => {
        tagSections[1].tags.push(App.createTag({ id: `genres:${type.id.toString()}`, label: type.name }));
      });
      filters.statuses.forEach((type) => {
        tagSections[2].tags.push(App.createTag({ id: `status:${type.id.toString()}`, label: type.name }));
      });
      filters.order.forEach((type) => {
        tagSections[4].tags.push(App.createTag({ id: `order:${type}`, label: type }));
      });
  ...
  var AsuraScans = class {
    constructor(cheerio) {
      ...
      this.sections = {
        "popular_today": {
          ...
          selectorFunc: ($) => $("div.group", $("h3:contains(Popular Today)")?.parent()?.next()?.next()),
          titleSelectorFunc: ($, element) => $("span.block", element).text().trim(),
          subtitleSelectorFunc: ($, element) => $("span.block", element)?.next()?.text().trim(),
          sortIndex: 10
        },
        "latest_update": {
          ...DefaultHomeSectionData,
      ...
      this.getMangaData = async (mangaId) => await this.loadRequestData(await this.getMangaUrl(mangaId));
    }
    async getSourceMenu() {
    ...
    async search(metadata, query) {
      ...
      metadata = !this.parser.isLastPage($, query?.title ? "search_request" : "view_more") ? { page: page + 1 } : void 0;
      return {
        metadata,
        manga
      };
